...
\section{Ausführung von BDT Tests innerhalb von MBT Tests}
...
Das endgültige Akzeptanzkriterium (\textit{then}) ist in \Gls{MBT} in einem ausreichend detaillierten Modell (wie es das Modell \ref{fig:modell_logisch} ist) eine Überprüfung in Code. Aus dem Modell wird nicht klar welche Überprüfung in diesem Blattknoten stattfindet. Hier zeigt sich wieder das Problem, das sich Logik in der Implementierung des Modells versteckt. Im \Gls{BDT} Test kommen meist mehrere Kriterien zum Einsatz die gleichzeitig erfüllt sein müssen. Es macht auch Sinn Überprüfungen außerhalb des gerade getesteten Kontexts zu machen. Im \Gls{BDT} Beispieltestfall in Abschnitt \ref{sec:bdt_bsp} wird genau das gemacht, indem eine lokale Überprüfung stattfindet (`Die Karte sollte wieder ausgegeben werden' und `Eine erklärende Fehlermeldung soll angezeigt werden'). Außerdem wird eine Überprüfung des Zentralsystems gemacht (`Der Kontostand sollte unverändert bleiben). Am Beispiel Graphwalker würden solche Überprüfungen natürlich möglich sein. Es könnten dieselben Funktionen aus der Testing API verwendet werden, die auch im \Gls{BDT} Test vorkommen. Solche kontextübergreifenden Überprüfungen lassen sich in einem grafischen Modell aber nur schwer darstellen. Es wäre wiederum entscheidende Logik im Testcode verborgen.\\
All diese Erkenntnisse zusammengefasst, erlauben eine Kombination von \Gls{BDT} und \Gls{MBT} die die Stärken beider Ansätze nutzt.
