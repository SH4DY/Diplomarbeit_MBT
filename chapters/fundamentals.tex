...
Gründe wieso Fehler auftreten sind mannigfaltig. Aus menschlicher Sicht können Fehler auf drei Ebenen auftreten\cite{vigenschow_objektorientiertes_2004}: \todo[inline]{RÜCKFRAGE ausformulierung der aufzählungspunkte zur Quelle}
...
\todo[inline]{RÜCKFRAGE Einleitung, 7 Qualitätsmerkmale nach ISO....}
...
Spricht man von \todo{dynamischen Testmethoden werden so eingeteilt, Siehe Sylabus ISTQB bzw. Basiswissen Softwaretest} Software Testmethoden, können diese grob in zwei Kategorien eingeteilt werden. Bei den sogenannten \textit{White-Box-Tests} werden Testfälle mit Kenntnis über die internen Vorgänge des zu testenden Systems entwickelt. Tests werden bewusst auf die Logik zugeschnitten. Üblicherweise ist der Testentwickler mit dem Quellcode des \ac{SUT} vertraut oder hat ihn sogar selbst verfasst. White-Box-Tests eignen sich sehr gut um festgestelltes Fehlverhalten im Quellcode zu lokalisieren bzw. um Fehler sehr früh und nah an der Entwicklung zu finden wie \citeauthor{spillner_basiswissen_2012} in \cite{spillner_basiswissen_2012} beschreibt. \todo{Hier ein Beispiel wie man texuell auf die Quelle eingeht.}\\
Der \textit{Black-Box-Test} ist das genaue Gegenteil \todo{würde ich nicht so bezeichnen, genaue Gegenteil finde ich nich gut}. Das zu testende System wird als uneinsehbarer Monolith verstanden. Ein Test kann lediglich Eingaben machen und die Ausgaben lesen. Die Black-Box-Sicht \todo{Bindestrichwörter} ähnelt der Sicht des Benutzers, da dieser ebenfalls keine Kenntnisse darüber besitzt was im inneren des Programms vorgeht. Um Black-Box-Testen zu betreiben, muss der Tester nur mit der Spezifikation des Programms vertraut sein. Dies ermöglicht, dass Black-Box-Testen an Teams abgegeben werden können, die nicht an der ursprünglichen Implementation beteiligt waren. Black-Box-Tests sind deshalb dazu geeignet Fehler in der Umsetzung der Spezifikation aufzudecken. Es ist kaum möglich im Detail festzustellen welche Komponente für das Fehlverhalten verantwortlich ist. Diese Art des Testens soll außerdem verhindern, dass die Entwickler, die das \ac{SUT} \todo{Artikel für SUT, Software under Test oder System under Test?} programmiert haben, bewusste oder unbewusste Annahmen treffen und dadurch das Testergebnis verfälschen\cite{spillner_basiswissen_2012}.  
...
In der ersten Teststufe des V-Modells werden die kleinsten Softwarebausteine einer systematischen Prüfung unterzogen. Abhängig von der verwendeten Programmiersprache sind diese kleinsten Bausteine Module oder Klassen. Dementsprechend spricht man dann von Modul- bzw. Unit-Tests\todo{RÜCKFRAGE typo bzw. Wording Bindestrich}\todo{Achte bitte auf Wording und Bedeutung von Modul, Komponenten, Unittest. Soll einheitlich in Deiner Arbeit verwendet werden.}.\\
...
\subsection{Agiles Testen}\todo{Die Subsectiokn Agiles testen beginnt mit einer Erklärung und Aufzählung von nicht agilen Projektvorgehen. Vielleicht das ganze in einen Paragaph packen oder irgendwie kennzeichen.}
...
\begin{itemize}
\item Organisation des Testings\todo[color=blue]{Wording}: Arbeitskräfte, Werkzeuge und Infrastruktur für das gesamte Testing-Vorhaben müssen organisiert werden.
...
Testing-Tätigkeiten \todo[color=blue]{Wording} können auf zwei Arten in den Ablauf eines Sprints integriert werden. Es können einzelne Tasks \todo[color=blue]{Wording} angelegt werden die eine Testing-Tätigkeit beschreiben oder die Kriterien zur Fertigstellung von Tasks werden dahingehend verändert, als das Testing-Tätigkeiten \todo[color=blue]{Wording} implizit dazugehören.\\
Das Monitoring von Testmetriken fließt nahtlos in die \textit{Continuous Integration} \todo{Muss beschrieben werden, was das ist} ein, die in jedem Scrum-Projekt essentiell ist. Dieser Umstand macht manuelles Überwachen des Testings unnötig\cite{linz_testing_2014}.\\
Auch in agilen Projekten soll die Software soll die Software \todo[color=blue]{Korrektur}, die aus dem V-Modell bekannten, Teststufen durchlaufen. Konträr zu sequentiellen Softwareprojekten werden die zuvor genannten Teststufen aber nicht hintereinander im Laufe des Projekt durchgeführt sondern parallel \todo{die teststufen werden ja trotzdem sequentiell in jeder Iteration durchlaufen oder?}in jeder Iteration. Im Optimalfall täglich und hochautomatisiert \cite{linz_testing_2014}.
...
Laut \todo{citeauthor, quellentag fehlerhaft} Vigenschow\cite{vigenschow_objektorientiertes_2004} vereinfacht dieser Ansatz, dass jeder Zustand mindestens einmal positiv und einmal negativ getestet wird. Während positive Testfälle gewährleisten, dass die geforderte Funktionalität ordnungsgemäß funktioniert, erlauben negative Testfälle die Überprüfung der Robustheit der Komponente. Typischerweise aufgedeckte Defekte sind die folgenden:
...
\todo[inline]{bei den abdeckungen sind keine Zeilenumbrüche}
...
\todo{RÜCKFRAGE loop coverage!, dazu gibts es Unterlagen}
...
B-Notation besitzt alle gängigen Operatoren, wie sie von den meisten modernen Programmiersprachen unterstützt werden. Dazu zählen Konjunktion, Disjunktion, Negation, Gleichheit und die arithmetischen Operatoren. Besonders hervorzuheben sind aber die logische Implikation ($\Rightarrow$) die logische Equivalenz ($\Leftrightarrow$). Außerdem sind die, aus der Prädikatenlogik bekannten, Quantoren verfügbar \cite{teschl_mathematik_2013}. Dazu gehören der Universal-Quantor ($\forall$) und der Existenz-Quantor ($\exists$).\\
...
Weiters gibt es Relationen die eine Beziehung zwischen den Elementen zweier Sets beschreiben (notiert als $A \leftrightarrow B$). Wobei eine konkrete Relation $r = {a_1 \rightarrow b_1} $ \todo{ausformulieren was in der Formel steht}
